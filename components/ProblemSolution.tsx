      ...
      <div className="sticky top-0 h-screen overflow-hidden">
        ...
        <div className="relative h-full flex flex-col items-center justify-center z-10">
          ...
          <div className="absolute inset-0 w-full h-full pointer-events-none flex items-center justify-center">
              ...
              >
                  <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col">
                          <span className="opacity-50 text-xs uppercase">{ps.humidity}</span>
                          <DataValue isSolution={isSolutionState} type="humid" critical={ps.critical} optimal={ps.optimal} />
                      </div>
                  </div>
              </motion.div>
              ...
              >
                  <div className="flex items-center justify-end gap-3">
                      <div className="flex flex-col">
                          <span className="opacity-50 text-xs uppercase">{ps.stability}</span>
                          <DataValue isSolution={isSolutionState} type="stab" critical={ps.critical} optimal={ps.optimal} />
                      </div>
                      <Activity />
                  </div>
              </motion.div>
          ...
          <div className="absolute bottom-20 text-center px-6 max-w-2xl [transform:translateZ(0)] w-full">
              {/* PROBLEM */}
              <div className={`transition-opacity duration-700 ease-in-out ${!isSolutionState ? 'opacity-100' : 'opacity-0'}`}>
                  <div className="inline-flex items-center gap-2 text-red-500 mb-4 px-4 py-1 rounded-full bg-red-500/10 border border-red-500/30">
                      ...
                      <span className="font-bold tracking-wider text-sm uppercase">{ps.criticalState}</span>
                  </div>
                  <h2 className={`text-4xl md:text-5xl font-bold mb-4 ${isDark ? 'text-white' : 'text-gray-900'}`}>{ps.fragileTitle}</h2>
                  <p className={`text-xl ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                    {ps.fragileDesc.split('\n').map((line, i) => (
                      <React.Fragment key={i}>{line}{i < ps.fragileDesc.split('\n').length - 1 && <br/>}</React.Fragment>
                    ))}
                  </p>
              </div>

              {/* SOLUTION */}
              <div className={`absolute inset-0 transition-opacity duration-700 ease-in-out ${isSolutionState ? 'opacity-100' : 'opacity-0'}`}>
                  <div className="inline-flex items-center gap-2 text-brand-highlight mb-4 px-4 py-1 rounded-full bg-blue-500/10 border border-blue-500/30">
                      <ShieldCheck size={18} />
                      <span className="font-bold tracking-wider text-sm uppercase">{ps.protectionActive}</span>
                  </div>
                  <h2 className={`text-4xl md:text-5xl font-bold mb-4 ${isDark ? 'text-white' : 'text-gray-900'}`}>{ps.solutionTitle}</h2>
                  <p className={`text-xl ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                    {ps.solutionDesc.split('\n').map((line, i) => (
                      <React.Fragment key={i}>{line}{i < ps.solutionDesc.split('\n').length - 1 && <br/>}</React.Fragment>
                    ))}
                  </p>
              </div>
          </div>
...
const DataValue: React.FC<{ isSolution: boolean; type: 'temp' | 'humid' | 'stab'; critical: string; optimal: string }> = ({ isSolution, type, critical, optimal }) => {
    ...
    useEffect(() => {
        ...
        if (!isSolution) {
            intervalId = window.setInterval(() => {
                ...
                if (type === 'stab') setVal(critical);
            }, 150);
        } else {
            if (type === 'temp') setVal("22.0°C");
            if (type === 'humid') setVal("45%");
            if (type === 'stab') setVal(optimal);
        }

        return () => {
            if (intervalId) clearInterval(intervalId);
        };
    }, [isSolution, type, critical, optimal]);
